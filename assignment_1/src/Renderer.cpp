...
namespace lidar_obstacle_detection
{
  void Renderer::addCircle(float centroid_x, float centroid_y, int id)
  {
    ...
    _viewer->addCircle(c_coeff, "c_" + std::to_string(id), 0);  
    _viewer->setShapeRenderingProperties(pcl::visualization::PCL_VISUALIZER_REPRESENTATION, 1000, "c_" + std::to_string(id));
    _viewer->setShapeRenderingProperties(pcl::visualization::PCL_VISUALIZER_COLOR, 0, 1, 0, "c_" + std::to_string(id));
    _viewer->setShapeRenderingProperties(pcl::visualization::PCL_VISUALIZER_OPACITY, 10000.0, "c_" + std::to_string(id));
  }

  void Renderer::addText(float centroid_x, float centroid_y, float centroid_z,const std::string&  id)
  {
    _viewer->addText3D(id, pcl::PointXYZ(centroid_x, centroid_y, 0), 0.5f, 255, 0, 0, "", 0);
  }

  void Renderer::removeShape(int id)
  {
    _viewer->removeShape("c_" + std::to_string(id), 0);
  }
  ...
  void Renderer::RenderRays(const Eigen::Vector3f& origin, const pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud)
  {
    for(pcl::PointXYZ point : cloud->points)
    {
      _viewer->addLine(
        ...
        point,
        1.0f, 0.0f, 0.0f, 
        "ray" + std::to_string(_rays_counter)
      );
      ++_rays_counter;
    }
  }
  ...
  void Renderer::ClearRays()
  {
    for(; _rays_counter > 0; _rays_counter--)
      _viewer->removeShape("ray" + std::to_string(_rays_counter));
  }
