...
*/
static constexpr int NPARTICLES = 0;
static constexpr const char circleID[] = "circle_id";
static constexpr const char reflectorID[] = "reflector_id";
...
void updateViewerReflector(const pcl::PointCloud<pcl::PointXYZI>& reflectorCenter)
{
	for(int i = 0; i < reflectorCenter.size(); i++)
	{
		...
		char label[32]{};
		std::snprintf(label, sizeof(label), "%s%d", reflectorID, i);
		
		renderer.updatePose(label, transform);
		renderer.updateShape(label, 1.0);
	}
}
...
void PointCloudCb(const sensor_msgs::PointCloud2ConstPtr& cloud_msg)
{
	...
	{
		char buff[32]{};
		std::snprintf(buff, sizeof(buff), "%s%d", reflectorID, i);
		renderer.updateShape(buff, 0.0);
	}
	...
	for(int i = 0; i < reflectorCenter.size(); i++)
		noisy_observations.emplace_back(0, reflectorCenter[i].x, reflectorCenter[i].y);
	...
	char label[32]{};
	std::snprintf(label, sizeof(label), "%s%d", circleID, NPARTICLES+1);

	showPCstatus(cloud_particles,particles);
	renderer.removeShape(label);
	renderer.addCircle(best_particles.back().x, best_particles.back().y, label, 0.3,1,0,0);
