...
DEFINE_double(
	robust_threshold,
  0.0,
  "Robust loss parameter. Set to 0 for normal squared error (no robustification)."
);
...
Circle CircleFitByCeres(std::vector<float>& aVecX, std::vector<float>& aVecY, float aRadious)
{
	double x = aVecX[0];
	double y = aVecY[0];
